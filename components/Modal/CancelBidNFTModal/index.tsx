...
export default function CancelBidNFTModal({ nft, show, onClose, bid }: Props) {
  ...
  useEffect(() => {
    if (error) {
      toast.error(`Error report: ${error.message}`, { autoClose: 1000, closeButton: true })
    }
  }, [error]);

  useEffect(() => {
    if (isSuccess) {
      toast.success(`Bid cancelled successfully`, { autoClose: 1000, closeButton: true })
      onClose?.()
    }
  }, [isSuccess]);
      ...
      <Modal.Body className="p-10">
        <div className="flex flex-col justify-center items-center gap-4 ">
          ...
          <div className="flex gap-4 w-full">
            ...
            <Button className="flex-1" onClick={handleCancelBid} loading={isLoading}>
              Yes
            </Button>
          </div>
        </div>
