     ...
     <div className="flex items-stretch gap-2 w-full" {...rest}>
       <div className="flex flex-col items-center">
         <div className={classNames(
            ...
            status === 'UPCOMING' && (isActive ? 'bg-warning/50' : 'bg-surface-medium')
         )}>
           <div className={classNames(
              'w-6 h-6 rounded-full flex items-center justify-center',
              status === 'ENDED' && 'bg-success',
              status === 'MINTING' && 'bg-white',
              status === 'UPCOMING' && (isActive ? 'bg-white' : 'bg-surface-soft')
           )}>
       ...
       <Collapsible
         ...
         <div className="rounded-2xl bg-surface-soft p-4 my-4 flex flex-col gap-2">
           <div className="flex items-center gap-1">
             ...
             {status === 'UPCOMING' && <p className="text-body-16 text-warning">Upcoming</p>}
             {status === 'MINTING' && <p className="text-body-16 text-success">Live</p>}
             {status === 'ENDED' && <p className="text-body-16 text-error">Ended</p>}
           </div>

           <p className="text-body-12 text-secondary">Start: {format(new Date(round?.start || 0), 'yyyy/M/dd - hh:mm a')}</p>
           <p className="text-body-12 text-secondary">End: {format(new Date(round?.end || 0), 'yyyy/M/dd - hh:mm a')}</p>
           ...
           <p className="text-body-12 text-secondary">Claimable
             at: {format(new Date(round?.claimableStart || 0), 'yyyy/M/dd - hh:mm a')}</p>
...
export default function ProjectMintSchedule({rounds, collection}: { rounds: Round[], collection: Collection }) {
