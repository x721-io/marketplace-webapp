...
export const useFilterByUser = create(
  devtools<NFTFilterByUserState & NFTFilterByUserAction>(
    (set, get) => ({
      ...{},
      createFiltersForUser: (userAddress) => set((state) => {
        return {
          ...state,
          [userAddress]: DEFAULT_FILTERS_BY_USER
        };
      }),
      toggleFilter: (mode, userAddress, bool) => set((state) => {
        ...
        return {
          [userAddress]: {
            ...
            [mode]: {
              ...
              showFilters: bool === undefined ? !state[userAddress][mode].showFilters : bool
            }
          }
        };
      }),
      setFilters: (mode, userAddress, filters) => set((state) => {
        if (!state[userAddress]) return state;
        return {
          [userAddress]: {
            ...state[userAddress],
      ...
      updateFilters: (mode, userAddress, filters) => set((state) => {
        if (!state[userAddress]) return state;
      ...
      resetFilters: (mode, userAddress) => set((state) => {
        if (!state[userAddress]) return state;
        return {
          [userAddress]: {
            ...state[userAddress],
            [mode]: {
              filters: {
                ...
                userAddress,
                creatorAddress: mode === 'created' ? state[userAddress][mode].filters.creatorAddress : undefined
              },
