...
export const useCalculateFee = ({
  ...
  const getFeeData = (data?: any[]): FeeData => {
    const [sellerFee, buyerFee, royaltiesFee, netReceived] = data && Array.isArray(data[0].result) ? data[0].result
...
export const useMarketTokenApproval = (token: Address, type: AssetType, totalCost: bigint) => {
  const wallet = useAuthStore((state) => state.profile?.publicKey);
  const marketContract =
    type === 'ERC721' ? contracts.erc721Market : contracts.erc1155Market;
